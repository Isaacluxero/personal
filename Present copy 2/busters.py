"""
Numbers at the bottom of the display are noisy distance readings to each remaining ghost.
"""
...
class GameState:
    """
    A GameState specifies the full game state, including the
...
def extract_inference_type(s):
    # Pattern to match the class name part right before "object at"
    pattern = r"\.(.*?)\s+object at"
    match = re.search(pattern, s)
    if match:
        return match.group(1)  # Return the matched group (class name)
    else:
        return None
filename = 'game_scores.csv'
def initialize_scores_df(filepath):
    if os.path.exists(filepath):

        # Load the existing scores DataFrame from the CSV file
        scores_df = pd.read_csv(filepath)
    else:

        # Initialize an empty DataFrame with columns if the CSV file doesn't exist
        scores_df = pd.DataFrame(columns=['Score', 'Inference', 'DateTime'])
    return scores_df

scores_df = initialize_scores_df(filename)

def update_scores(score, inference, scores_df, filepath):
    current_datetime = datetime.now()  # Current date and time
    # Create a new DataFrame for the new row
    new_row_df = pd.DataFrame({'Score': [score], 'Inference': [inference],'DateTime': [current_datetime]})
    # Use pd.concat to append the new row DataFrame to the existing scores DataFrame
    ...
    return scores_df
def print_highest_score(scores_df):
...
def runGames( layout, pacman, ghosts, display, numGames, maxMoves=-1):
    ...
    for i in range(numGames ):
        ...
        game.run()
        update_scores(game.state.getScore(), extract_inference_type(str(pacman.inferenceModules)), scores_df, filename)
        games.append(game)

    print_highest_score(scores_df)
    if numGames > 1:
