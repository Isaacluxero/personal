...
public class TERenderer {
     ...
     */
    public void renderFrame(TETile[][] world, int LIVES) {
        ...
        renderInterface(world, LIVES);
        StdDraw.show();
    }

    private void renderInterface(TETile[][] world, int LIVES) {
        int mouseX = (int) StdDraw.mouseX();
        int mouseY = (int) StdDraw.mouseY();
        StdDraw.setPenColor(Color.yellow);
        if (mouseX >= xOffset && mouseX <= width + xOffset && mouseY >= yOffset && mouseY + yOffset <= height) {
            TETile tile = world[mouseX - xOffset][mouseY - yOffset];
            StdDraw.text(12, 0.5, tile.description());
            StdDraw.filledSquare(mouseX + 0.5, mouseY + 0.5, 0.5);
        }
        StdDraw.text(width / 2, 0.5, new Date().toString());
        StdDraw.textLeft(width - 12, 0.5, "Lives: " + LIVES);
    }
}
