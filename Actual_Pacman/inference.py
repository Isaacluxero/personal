...
def constructBayesNet(gameState: hunters.GameState):
    ...
    "*** YOUR CODE HERE ***"
    variables = [PAC, GHOST0, GHOST1, OBS0, OBS1] #DONE
    ...
    "*** END YOUR CODE HERE ***"
    #The function creates an empty Bayesian network object
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        "*** YOUR CODE HERE ***"
        currentList = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        for eli_varv in eliminationOrder:
            currentList, joinedFactor = joinFactorsByVariable(currentList, eli_varv)
            if len(joinedFactor.unconditionedVariables()) == 1:
                continue
            eliminate_f = eliminate(joinedFactor, eli_varv)
            currentList.append(eliminate_f)
        joined_f = joinFactors(currentList)
        return normalize(joined_f)    
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        "*** END YOUR CODE HERE ***"
        #This line of code is from a class method that updates the agent's belief distribution over the possible locations of a hidden state
        self.beliefs.normalize()
